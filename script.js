...
function drawPizza() {
  for (let i = 0; i < pizzaArray.length; i++) {
    ...
    if (pizzaArray[i].x < 0) {
      pizzaArray[i].x = 1800;
      pizzaArray[i].y = Math.floor(Math.random() * 1000);
    }

    if (
      playerImgX < pizzaArray[i].x + pizzaArray[i].width &&
...
function drawObstacle() {
  for (let i = 0; i < dcArray.length; i++) {
    ...
    if (dcArray[i].x < 0) {
      dcArray[i].x = 1800;
      dcArray[i].y = Math.floor(Math.random() * 1000);
    }

    if (
      playerImgX < dcArray[i].x + dcArray[i].width &&
...
function gameOverFun() {
  ...
  console.log("******THANKS FOR PPAYINNG****");
}
